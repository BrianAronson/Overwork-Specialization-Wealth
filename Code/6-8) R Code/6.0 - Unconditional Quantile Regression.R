#NOTE TO SELF:
#Manually changing a quantile to overlap with essentially tests "what does it take to get to 0." If anything, the goal should be to make the quantile overlap with a number slightly above 0; not below. 
...
#1 - prep data
    ...
    #b - identify variables of interest
        vars<- c("Networth","HSGraduate","SomeCollege","Bachelors","AdvancedDegree","Black",
                 ...
                 "Age","AgeSquared","Children","ChildrenSquared","Year1","HeadWorkHoursOver40","HomemakerSpouse","INCOME")
  ...
  #wealth
    ...
    for(i in 1:length(lscf)){
       # tempdf$tincome<-tempdf$INCOME
        ...
        rifreg <- urq(formula=(Networth~tincome+INCOME+HSGraduate+SomeCollege+Bachelors+
    ...
    #export to csv
        write.csv(uqrregres,("uqrregres1919.csv"))
    }
...
#home equity    
    ...
    for(i in 1:length(lscf)){
        ...
        rifreg <- urq(formula=(HomeEquity~INCOME+HSGraduate+SomeCollege+Bachelors+
                                 AdvancedDegree+ Black+Latino+Other+Married+Separated+Divorced+
                                 Widowed+LivingWithPartner+Age+AgeSquared+Children+
                                 ChildrenSquared+Year1+HeadWorkHoursOver40+HomemakerSpouse+
                                 HeadWorkHoursOver40*HomemakerSpouse + Year1*HeadWorkHoursOver40),
        ...
        names(as.data.frame(rifreg$coefficients))
        round(rifreg$coefficients[,2],2)
    ...
    #rename variables for SAS (i.e. get rid of weird characters)
        ...
        uqrregres$Parameter<-gsub("[():]","",uqrregres$Parameter)
    #sort by quantile
        uqrregres$Quantile[uqrregres$Quantile>.1 & uqrregres$Quantile<.5]<-.25
        uqrregres<-uqrregres[order(uqrregres$Quantile),]
    #remove NA parameters; replace variables names with those that are compatible in SAS.
        uqrregres[1:10,]
        uqrregres<-uqrregres[!is.na(uqrregres$Estimate),]
        ...
        View(uqrregres)
    #export to csv
        write.csv(uqrregres,("uqrregres1919Home.csv"))
    }
...
#stocks
    ...
    for(i in 1:length(lscf)){
        ...
        #set .75 quantile so that intercept is 0
            ...
            # newquant<-rnk[round(tempdf$StockValue,4)==round(quant,4)][1]/max(rnk,na.rm=T)
            newquant<-rnk[round(tempdf$StockValue,4)==round(min(tempdf$StockValue[tempdf$StockValue>0]),4)][1]/max(rnk,na.rm=T)+.001

        rifreg <- urq(formula=(StockValue~INCOME+HSGraduate+SomeCollege+Bachelors+
                                 AdvancedDegree+ Black+Latino+Other+Married+Separated+Divorced+
                                 Widowed+LivingWithPartner+Age+AgeSquared+Children+
                                 ChildrenSquared+Year1+HeadWorkHoursOver40+HomemakerSpouse+
                                 HeadWorkHoursOver40*HomemakerSpouse + Year1*HeadWorkHoursOver40),
    ...
    #rename variables for SAS (i.e. get rid of weird characters)
        ...
        uqrregres$Parameter<-gsub("[():]","",uqrregres$Parameter)
    #sort by quantile
        uqrregres$Quantile[uqrregres$Quantile>.5 & uqrregres$Quantile<.9]<-.75
        uqrregres<-uqrregres[order(uqrregres$Quantile),]
    #remove NA parameters; replace variables names with those that are compatible in SAS.
        uqrregres[1:10,]
        uqrregres<-uqrregres[!is.na(uqrregres$Estimate),]
        ...
        uqrregres$Parameter[uqrregres$Parameter=="Year1HeadWorkHoursOver40"]<-"Year1HeadWorkHoursOve"
    #export to csv
        write.csv(uqrregres,("uqrregres1919Stocks.csv"))
    }
